...
1. **Given** specifies WHO (user type, permissions) and WHAT STATE exists (data, config)
   - Bad: `Given a user` → Good: `Given an authenticated user with admin role`
2. **When** describes ONE user action — not implementation details
   - Bad: `When POST /api/search` → Good: `When they search with filters category="Electronics"`
3. **Then** uses specific, assertable values — never "it works"
   - Bad: `Then results are returned` → Good: `Then results contain only Electronics products under $500`
