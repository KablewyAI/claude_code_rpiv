...
### Step 5: Invariants & Business Rules Check (MANDATORY)

Check implementation against the institutional knowledge files:

- [ ] **INVARIANTS.md**: Does this change violate any system invariant? Pay special attention to data isolation, auth boundaries, and error handling invariants.
- [ ] **BUSINESS_RULES.md**: Does this change conflict with any documented business rule? Would it accidentally "fix" something that looks wrong but is correct?
- [ ] **Referenced research**: If the plan references a research doc, check its `status` field. If `superseded`, the plan may be based on outdated findings.
- [ ] **thoughts/shared/lessons/**: Are there lessons learned from past debugging that apply to this change?

### Step 6: Security Review (MANDATORY)
...
### Step 7: Production Failure Mode Review (MANDATORY)
...
- [ ] No system invariants violated (`INVARIANTS.md`)
- [ ] No business rules contradicted (`BUSINESS_RULES.md`)
- [ ] Referenced research is current (not `status: superseded`)
- [ ] No lessons from `thoughts/shared/lessons/` apply and were ignored
