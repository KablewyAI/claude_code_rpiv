...
- Is it a config issue? (Check config files, env vars, secrets)
...
After completing your investigation, assess whether this debugging session revealed a pattern worth preserving. If ANY of the following are true, suggest creating a lesson:

- The root cause was a non-obvious interaction between systems
- The same mistake has been made before (check `thoughts/shared/lessons/` and `MEMORY.md`)
- The debugging required knowledge that wasn't documented anywhere
- The fix location was different from where the symptom appeared
- A future developer (or AI) would likely make the same wrong assumption
...
This debugging session revealed [pattern]. Consider capturing it:

/context_dump or manually add to:
- `thoughts/shared/lessons/YYYY-MM-DD_<slug>.md` — What happened, why, what changed, how to spot it
- `BUSINESS_RULES.md` — If it's a business rule that wasn't documented
- `INVARIANTS.md` — If it's a system invariant that was violated
```
