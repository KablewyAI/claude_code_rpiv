...
| [Example: Search for items] | [Authenticated, has read access, data exists] | [Filtered results, paginated] |
...
|---|---|---|---|
| [e.g., Primary DB] | [e.g., User.findById throws] | [e.g., Generic catch returns 401] | [e.g., None — DB down looks like auth failure] |
...
- **API compatibility**: [works with our stack? Runtime compatible? ESM?]
...
If this research CONTRADICTS findings from a prior research document, note it explicitly:

| Prior Doc | Prior Finding | Correction | Why It Was Wrong |
|-----------|--------------|------------|-----------------|
| [path] | [what it said] | [what's actually true] | [how the error occurred] |

If no prior research exists on this topic, delete this section.

**IMPORTANT**: If you find a prior research doc with wrong conclusions, update its YAML frontmatter to add `status: superseded` and `superseded_by: <path to this doc>`. This prevents future sessions from inheriting faulty logic.
...
- [x] Found config in env.js and config files
