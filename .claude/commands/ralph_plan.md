...
- [ ] **"Why Not" section filled out** — rejected approaches documented with specific reasons (not just "we prefer custom")
...
Before diving into phases, briefly note what you considered and rejected.
**REQUIRED for every plan.** Takes 60 seconds. Highest-value knowledge for future sessions.

- We considered [approach X] but rejected it because [specific reason]
- We considered [approach Y] but rejected it because [specific reason]
- [If obvious approach was skipped]: The obvious approach ([what]) doesn't work here because [why]
...
For each major design decision, document at least one alternative with detailed tradeoffs.
...
1. **Given** specifies WHO (user type, permissions, org context) and WHAT STATE exists (data, config)
   - Bad: `Given a user`
   - Good: `Given an authenticated user with admin role in org "acme-corp"`
...
2. **When** describes ONE user action or system event — not implementation details
   ...
   - Good: `When they search for products with filters category="Electronics", maxPrice=500`

3. **Then** uses specific, assertable values — never "it works" or "results are returned"
   - Bad: `Then results are returned`
   - Good: `Then results contain only products where category="Electronics" AND price < 500`
...
6. **No implementation leaking** — describe behavior, not database operations or internal APIs
   - Bad: `Given a row exists in the products table with id=123`
   - Good: `Given a product exists in category "Electronics" priced at $450`
...
- `Then results contain 3 products all in Electronics` → `assert(results.length === 3); results.forEach(r => assert(r.category === 'Electronics'))` — specific, falsifiable
...
8. **Why Not**: Is the "Why Not" section filled out with specific rejected approaches?
9. **Alternatives**: Does every significant decision have at least one documented alternative?
10. **API Safety**: Are all API changes classified as growth or breakage?
11. **Production Failure Modes**: For every phase that calls a DB/API/service, is there a scenario for when that dependency is down? Does the error differentiate from user error?
12. **Build vs. Buy**: If the research found viable packages, is there a documented decision on whether to use them or build custom? Is the rationale specific (not just "we prefer custom")?
