...
Safety and session management hooks. Configured in `settings.json`.
...
| Hook | Event | Purpose |
|------|-------|---------|
| `auto-handoff-on-compact.sh` | PreCompact | Auto-generates a handoff document before context compaction, preserving session state |
| `context-usage-reminder.sh` | Stop | Warns at 70% context usage, urges handoff at 85% |
| `statusline-context-tracker.sh` | StatusLine | Tracks context window usage, writes state for other hooks, shows usage in status bar |
