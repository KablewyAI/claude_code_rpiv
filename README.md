...
- [Command Reference](#command-reference) — All 20 commands with descriptions
...
These are common anti-patterns in AI-assisted development. Each one is preventable with RPIV discipline.

### 1. The Wrong Code Path

**Pattern:** AI finds a function that matches the bug description, fixes it, and ships. The bug persists because the fix was in a code path that doesn't execute in the reported scenario.

**Example:** A migration bug is reported. The AI finds a migration function and fixes the SQL. But the affected flow uses a *different* migration path — the fix was correct code in the wrong file.

**What RPIV prevents:** The research phase traces the *actual execution path* from trigger to outcome, identifying all code paths — not just the first one grep finds.

### 2. The Incomplete Fix

**Pattern:** AI fixes the reported symptom but misses other states that trigger the same class of error.

**Example:** "No such table" is reported. A CREATE TABLE migration is added. Tests pass. Deployed. New error: "no such column." The table existed in some environments but was missing a column added in a later migration. The fix handled "no table" but not "table exists, column missing."

**What RPIV prevents:** The plan phase requires enumerating *all* possible states, not just the one in the bug report. The DoD requires verification commands that cover each state.

### 3. The Diagnosis That Wasn't

**Pattern:** AI picks the easiest explanation, treats it as fact, and waits instead of investigating further.

**Example:** A feature is broken. The AI checks the relevant service, finds nothing wrong, and concludes "the latest change isn't deployed yet." A day passes. Still broken. The real issue was stale cached data — the AI diagnosed a deployment problem because it was the simplest explanation, not the correct one.

**What RPIV prevents:** The research phase requires *ruling out alternatives* with evidence. "Not deployed" would need verification (checking deployment logs/status) before being accepted as the root cause.

### 4. The Identifier That Never Existed

**Pattern:** AI infers an identifier from naming conventions instead of verifying it against documentation.

**Example:** A third-party API integration uses the endpoint `v3.0-preview`. Every call returns 404. The correct identifier is `v3-preview` (no `.0`). The AI assumed the naming pattern from a similar API without checking the actual docs.

**What RPIV prevents:** The research phase requires verifying facts against source documentation — not inferring from patterns. A research doc with "Endpoint: `v3-preview` (verified: [link to docs])" would have caught this.

### 5. The Low-Signal Test Suite

**Pattern:** AI writes tests that pass but don't verify behavior. Everything looks green. The feature doesn't actually work.

**Example:** Tests mock the entire service layer and only verify functions were called with the right arguments. `expect(mockService.create).toHaveBeenCalledWith(...)` passes, but the real service throws an error the mocks hide.

**What RPIV prevents:** The validation phase checks test *quality*, not just test *results*. A test that mocks everything and asserts `toHaveBeenCalled` is low-signal — it passes regardless of whether the feature works.

### The Pattern

Every failure above shares a common structure: **a step was skipped because it felt unnecessary.** The full code path wasn't traced because a plausible one was found. All database states weren't checked because one was reproduced. The API docs weren't verified because the naming convention seemed obvious. Test quality wasn't questioned because the tests passed.
...
These hooks eliminate the need for manual handoff discipline. They monitor context usage and auto-preserve session state before compaction destroys it.

| Hook | Event | What It Does |
|------|-------|--------------|
| `statusline-context-tracker.sh` | StatusLine | Writes context window usage (%) to a state file after each turn. Shows color-coded indicator in status bar. |
| `context-usage-reminder.sh` | Stop | At 70%: warns Claude to suggest `/create_handoff`. At 85%: **spawns a background `claude -p` instance** that reads the transcript and writes a rich handoff — completely non-blocking. |
| `auto-handoff-on-compact.sh` | PreCompact | Last resort safety net. If compaction triggers and no handoff exists yet, parses the transcript JSONL with a shell script and writes a crude handoff. |

**How they work together:**
```
Each turn → StatusLine writes context % to /tmp state file
         → Stop hook reads state file
              → < 70%: silent
              → ≥ 70%: "Consider /create_handoff" (message injection)
              → ≥ 85%: Spawns background: claude -p "write handoff..."
                       → Non-blocking, full tool access
                       → Reads transcript, writes rich handoff
                       → Main session continues uninterrupted
         → If compaction triggers before background Claude finishes:
              → PreCompact hook writes crude shell-parsed handoff as fallback
```

**Three tiers of handoff quality:**
1. **Best:** Manual `/create_handoff` — human-guided, captures intent and priorities
2. **Good:** Background `claude -p` at 85% — LLM-generated, reads full transcript, non-blocking
3. **Crude:** PreCompact shell script — just file parsing, no LLM, but always runs
...
RPIV's institutional memory goes beyond the `thoughts/` directory. The full knowledge system has seven layers, each serving a different purpose:

| Layer | File | What It Captures | When to Write |
|-------|------|-----------------|---------------|
| **Business Rules** | `BUSINESS_RULES.md` | Non-obvious business logic — "this looks wrong but is correct because..." | When you encounter a rule that isn't self-evident from code |
| **System Invariants** | `INVARIANTS.md` | Things that must NEVER break, regardless of what feature is being built | When you discover a global constraint (not feature-specific) |
| **Lessons Learned** | `thoughts/shared/lessons/` | Debugging patterns worth preserving — what happened, why, how to spot it | After `/debug` reveals a non-obvious failure mode |
| **Research** | `thoughts/shared/research/` | Full investigations with file:line refs, data flow traces, "why not" context | During `/ralph_research` |
| **Plans** | `thoughts/shared/plans/` | Decisions with alternatives, acceptance criteria, rollback strategies | During `/ralph_plan` |
| **Validations** | `thoughts/shared/validations/` | What was checked, what passed, what was caught | During `/validate_plan` |
| **Glossary** | `thoughts/shared/glossary.md` | Disambiguates overloaded terms (Handler, Database, Server, etc.) | When a term means different things in different contexts |

The key insight: **knowledge degrades as it flows through the pipeline**. Research docs are rich in "why." Plans capture "what and how." Code captures "what." By the time something is implemented, the reasoning behind it has been progressively stripped away — unless you systematically capture it.

#### Guarding Against Inherited Faulty Logic

When AI sessions reference prior research docs, they inherit the conclusions — including wrong ones. RPIV guards against this in three ways:

1. **Research correction tracking**: The research template includes a "Corrections to Prior Research" section. When new research contradicts prior findings, it explicitly marks the old doc as `status: superseded` in its YAML frontmatter and documents what changed and why.

2. **Validation cross-check**: The validation step checks whether referenced research is still current (not superseded) and whether the implementation contradicts any documented business rules or system invariants.

3. **Context dump sessions**: The `/context_dump` command runs a structured knowledge extraction interview — it scans for gaps in business rules, invariants, and lessons learned, then generates questions for the human to answer. This surfaces knowledge that's in the developer's head but hasn't been written down yet.

#### The "Why Not" Section

Every plan includes a lightweight "Why Not" section at the top — 2-3 bullets noting what was considered and rejected, before the detailed phases begin. This takes 60 seconds to write and is the single highest-value knowledge for future sessions:

```markdown
## Why Not (Quick Reference)
- We considered WebSocket streaming but rejected it because SSE is simpler and our infrastructure already handles it
- We considered a shared multi-tenant database but rejected it because per-tenant isolation is a compliance requirement
- The obvious approach (polling) doesn't work here because latency requirements are under 500ms
```
...
After investigation, suggests creating a lesson if the debugging revealed a non-obvious pattern.
```

### Knowledge Extraction

#### `/context_dump` — Structured Knowledge Interview
```
Scans for gaps in BUSINESS_RULES.md, INVARIANTS.md, and lessons/.
Generates 10-15 specific questions organized by system. Documents answers to the correct files.
Use quarterly or after major features ship.
```

#### `/prompt_review` — Prompting Feedback
```
Analyzes the conversation session for effective and ineffective prompting patterns.
Provides actionable feedback on how to communicate more effectively with the AI.
```
...
│   │   ├── validate_plan.md           # /validate_plan (checks invariants & business rules)
│   │   ├── iterate_plan.md            # /iterate_plan
│   │   ├── ralph.md                   # /ralph (smart router)
│   │   ├── ralph_research.md          # /ralph_research (tracks corrections to prior research)
│   │   ├── ralph_plan.md              # /ralph_plan (includes "Why Not" section)
...
│   │   ├── debug.md                   # /debug (suggests lessons after investigation)
│   │   ├── tdd.md                     # /tdd
│   │   ├── creative_thinking.md       # /creative_thinking
│   │   ├── context_dump.md            # /context_dump (knowledge extraction interview)
│   │   └── prompt_review.md           # /prompt_review (prompting feedback)
│   ├── hooks/
...
│   │   ├── auto-handoff-on-compact.sh     # PreCompact: auto-generate handoff
│   │   ├── context-usage-reminder.sh      # Stop: warn at 70%/85% context usage
│   │   └── statusline-context-tracker.sh  # StatusLine: track context % for hooks
...
└── thoughts/
    └── shared/
        ├── glossary.md                # Disambiguates overloaded terms
        ├── research/                  # Research documents
        ├── plans/                     # Implementation plans
        ├── validations/               # Validation reports
        ├── handoffs/                  # Session handoff docs
        ├── lessons/                   # Debugging patterns worth preserving
        ├── reviews/                   # First-principles adversarial reviews
        └── implementations/           # Before/after records of significant refactors
```
...
Manual discipline is unreliable — you get absorbed in the work and forget. RPIV includes three hooks that automate handoff discipline so you don't have to remember:
...
| Tier | Trigger | Quality | Blocking? |
|------|---------|---------|-----------|
| Manual `/create_handoff` | You remember | Best — human-guided, captures intent | Inline (part of conversation) |
| Background `claude -p` | 85% context | Good — LLM reads full transcript | Non-blocking (background process) |
| Shell script parse | PreCompact event | Crude — just `jq` on JSONL | Non-blocking (async hook) |

```
Session flow with automated handoff discipline:

  Turn 1-N  →  StatusLine: "🟢 45%"  (silent, working normally)
  Turn N+1  →  StatusLine: "🟡 72%"  →  Stop hook: "Consider /create_handoff"
  Turn N+5  →  StatusLine: "🔴 87%"  →  Stop hook spawns: claude -p "write handoff..."
                                         (background, non-blocking, full tool access)
  Turn N+8  →  Background Claude writes rich handoff to thoughts/shared/handoffs/
  Turn N+10 →  If compaction triggers and no handoff exists yet:
             →  PreCompact hook: crude shell-script fallback
             →  Post-compaction: Claude sees "Auto-handoff created at <path>"
```

To configure these hooks, see the [Session Lifecycle Hooks](#session-lifecycle-hooks-automatic-handoff-discipline) section above.
